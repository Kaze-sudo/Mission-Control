...
const VALID_RUNTIMES = new Set<RuntimeId>(['openclaw', 'hermes', 'claude', 'codex', 'opencode', 'gamut'])
...
export async function POST(request: NextRequest) {
  ...
  if (action === 'install') {
    ...
    if (!runtime || !VALID_RUNTIMES.has(runtime)) {
      return NextResponse.json({ error: 'Invalid runtime. Use: openclaw, hermes, claude, codex, opencode, gamut' }, { status: 400 })
    }
