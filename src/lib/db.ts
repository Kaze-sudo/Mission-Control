...
function seedAdminUserFromEnv(dbConn: Database.Database): void {
  ...
  if (count > 0) {
    // Env-seeded admin re-sync: the seed only ran on an empty users table, so
    // a changed AUTH_PASS/AUTH_PASS_B64 silently drifted from the stored hash
    // and logins started failing with "Invalid credentials". The env is the
    // source of truth for the seeded account, so re-sync the hash — loudly.
    if (password) syncEnvSeededAdminPassword(dbConn, username, password)
    return
  }
  ...
  dbConn.prepare(`
    INSERT OR IGNORE INTO users (username, display_name, password_hash, role, auth_source)
    VALUES (?, ?, ?, ?, 'env-seeded')
...
/**
 * Re-sync the env-seeded admin's password hash when AUTH_PASS/AUTH_PASS_B64
 * changes between boots. Only touches the AUTH_USER account, and only when it
 * is a local-provider user (never OAuth/proxy-provisioned identities).
 */
function syncEnvSeededAdminPassword(dbConn: Database.Database, username: string, password: string): void {
  const row = dbConn.prepare('SELECT id, password_hash, provider FROM users WHERE username = ?')
    .get(username) as { id: number; password_hash: string; provider?: string | null } | undefined
  if (!row) return
  if ((row.provider || 'local') !== 'local') return
  if (verifyPassword(password, row.password_hash)) return

  const now = Math.floor(Date.now() / 1000)
  dbConn.prepare(
    "UPDATE users SET password_hash = ?, auth_source = 'env-seeded', updated_at = ? WHERE id = ?",
  ).run(hashPassword(password), now, row.id)
  logger.warn(
    `Admin "${username}" password no longer matches AUTH_PASS/AUTH_PASS_B64 — re-synced the stored hash from env. ` +
    'If this was not intended, rotate AUTH_PASS back or change the password in the UI (which becomes the source of truth until AUTH_PASS changes again).'
  )
}
