...
export function createDeepReviewMission(input: {
  ...
  // Resolve the mission's project before building metadata.
  const db = getDatabase()
  ...
  const ticketNo = allocateProjectTicket(projectId, input.workspaceId)
  ...
  const metadata = {
    agentos_resource_review: {
      ...
      fingerprint,
      reviewer: input.reviewer || item.deepReview?.reviewer || null,
    },
    agentos: {
  ...
  const insert = db.prepare(`INSERT INTO tasks (
    title, description, status, priority, project_id, project_ticket_no, assigned_to,
    created_by, created_at, updated_at, tags, metadata, workspace_id
  ) VALUES (?, ?, 'inbox', ?, ?, ?, NULL, ?, ?, ?, ?, ?, ?)`).run(
    `Deep review: ${item.probableName}`,
    description,
    priority,
    projectId,
    ticketNo,
    ...
    now,
    now,
    JSON.stringify(['agentos-resource-review']),
  ...
  let reviewer = metadata.agentos_resource_review.reviewer as string | null
  let routing: { externalAgentId?: string; agentName?: string; platoonId?: string; routingAgentName?: string } | null = null
  if (projectId) {
    try {
      // Normal AgentOS project/specialist routing. routeTaskWithinProject merges
      // its own agentos_routing + agentos_resources into the task metadata.
      const routeResult = routeTaskWithinProject({ taskId, workspaceId: input.workspaceId, actor })
      ...
      if (routed && routeResult.selected) {
        ...
        reason = routeResult.reason || 'No eligible reviewer yet (mission held for manual assignment)'
      }
    } catch (err) {
  ...
  db_helpers.logActivity('agentos_deep_review_routed', 'task', taskId, actor,
    ...
    { review_id: reviewId, task_id: taskId, routed }, input.workspaceId)

  return { taskId, reviewId, workspaceId: input.workspaceId, projectId, taskStatus: routed ? 'assigned' : 'inbox', routed, reason, reviewer, fingerprint }
}

function allocateProjectTicket(projectId: number | null, workspaceId: number): number | null {
  if (!projectId) return null
  const db = getDatabase()
  db.prepare('UPDATE projects SET ticket_counter = ticket_counter + 1, updated_at = unixepoch() WHERE id = ? AND workspace_id = ?')
    .run(projectId, workspaceId)
  const row = db.prepare('SELECT ticket_counter FROM projects WHERE id = ? AND workspace_id = ?')
    .get(projectId, workspaceId) as { ticket_counter: number } | undefined
  return row?.ticket_counter ?? null
}
...
export function ingestDeepReviewResult(input: {
  ...
  if (!parsed.ok || !parsed.value) {
    ...
    setQueueDeepReviewState(root, reviewId, 'FAILED', { error: errorText, failed_at: new Date().toISOString() })
  ...
  db_helpers.logActivity('agentos_deep_review_complete', 'task', input.taskId, 'agentos',
    ...
    { review_id: reviewId }, input.workspaceId)
...
export function reconcileDeepReviewMissions(rootInput?: string): { ok: boolean; message: string; ingested: number; stale: number; invalid: number; running: number } {
  ...
  for (const task of tasks) {
    ...
    if (['COMPLETE', 'FAILED', 'STALE'].includes(String(review.state || ''))) continue
    try {
...
export function retryDeepReviewMission(input: {
  ...
  const projectId = input.projectId ?? metadata.agentos_resource_review?.project_id ?? null
  const now = Math.floor(Date.now() / 1000)
  const actor = input.actor || 'agentos'
  ...
  db_helpers.logActivity('agentos_deep_review_retried', 'task', existing.id, actor,
    ...
    { review_id: input.reviewId, retries }, input.workspaceId)

  return { taskId: existing.id, reviewId: input.reviewId, workspaceId: input.workspaceId, projectId, taskStatus: routed ? 'assigned' : 'inbox', routed, reason, reviewer, fingerprint }
}
