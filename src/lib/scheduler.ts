...
import { reconcileObjectiveStatuses } from './objective-planning'
    ...
    try {
      const result = id === 'auto_backup' ? await runBackup()
        ...
        : id === 'task_dispatch' ? await autoRouteInboxTasks().then(async (routeResult) => {
            ...
            const objectiveChanges = reconcileObjectiveStatuses()
            const objectiveResult = { ok: true, message: objectiveChanges.length ? `${objectiveChanges.length} objective(s) reconciled` : '' }
            const brokerResult = brokerAgentOSDispatchQueue()
            const dispatchResult = await dispatchAssignedTasks()
            const parts = [reconcileResult.message, routeResult.message, deepReviewResult.message, objectiveResult.message, brokerResult.message, dispatchResult.message].filter(m => m && !m.includes('No ') && !m.includes('none completed'))
...
export async function triggerTask(taskId: string, workspaceId?: number): Promise<{ ok: boolean; message: string }> {
  ...
  if (taskId === 'task_dispatch') return autoRouteInboxTasks().then(async (r) => { const c = await reconcileDeferredTaskCompletions(); const dr = reconcileDeepReviewMissions(); const ocChanges = reconcileObjectiveStatuses(); const oc = { ok: true, message: ocChanges.length ? `${ocChanges.length} objective(s) reconciled` : '' }; const b = brokerAgentOSDispatchQueue(workspaceId); const d = await dispatchAssignedTasks(); return { ok: r.ok && c.ok && b.ok && d.ok, message: [c.message, r.message, dr.message, oc.message, b.message, d.message].filter(m => m && !m.includes('No ') && !m.includes('none completed')).join(' | ') || 'No tasks' } })
